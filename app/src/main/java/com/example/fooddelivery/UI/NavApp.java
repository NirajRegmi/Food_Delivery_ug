...
import static com.example.mylibrary.SharedClass.STATUS_DELIVERED;
import static com.example.mylibrary.SharedClass.STATUS_DELIVERING;
import static com.example.mylibrary.SharedClass.STATUS_DISCARDED;
import static com.example.mylibrary.SharedClass.STATUS_UNKNOWN;
import static com.example.mylibrary.SharedClass.orderToTrack;
import static com.example.mylibrary.SharedClass.orderToTrack;
...
import com.example.mylibrary.StarItem;
...
public class NavApp extends AppCompatActivity implements
    ...
    public void getUserInfo() {
        DatabaseReference myRef = FirebaseDatabase.getInstance().getReference(fooddelivery_PATH).child(ROOT_UID);

        myRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                user = dataSnapshot.child("fooddelivery_info").getValue(User.class);
                Log.d("user", ""+user);
            }
    ...
    private void onRefuseOrder (){
        ...
        if(orderToTrack!=null){
            for(HashMap.Entry<String, Integer> entry : orderToTrack.entrySet()){
                Query query = FirebaseDatabase.getInstance().getReference(fooddelivery_PATH).child(ROOT_UID).child("orders").child(entry.getKey());
    ...
    private void setRated (String orderKey, boolean bool) {
        DatabaseReference myRef2 = FirebaseDatabase.getInstance().getReference(fooddelivery_PATH).child(ROOT_UID).child("orders").child(orderKey);
